...
abstract class DrugInteractionApiService {
  ...
  Future<InteractionAnalysis> checkMedicationInteractions(@Body() List<String> medicationIds);

  /// Check interactions for specific RxNorm codes
  @POST('/drug-interactions/check-rxnorm')
  Future<InteractionAnalysis> checkRxNormInteractions(@Body() List<String> rxNormCodes);

  /// Validate medication against RxNorm database
  @POST('/drug-interactions/validate-rxnorm')
  Future<MedicationEnrichmentResponse> validateRxNorm(@Body() Map<String, String> request);

  /// Enrich medication data with RxNorm information
  @POST('/drug-interactions/enrich-medication')
  Future<MedicationEnrichmentResponse> enrichMedicationData(@Body() Map<String, dynamic> medicationData);

  /// Search RxNorm database for medication
  @GET('/drug-interactions/search-rxnorm')
  Future<RxNormSearchResponse> searchRxNorm(@Query('term') String searchTerm, @Query('limit') int? limit);
