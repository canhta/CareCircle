...
class MedicationCacheManager {
  static final MedicationCacheManager _instance =
  ...
  void cacheMedicationList(String cacheKey, List<Medication> medications) {
    ...
    _logger.debug(
      'Cached medication list: $cacheKey (${medications.length} items)',
    );
  }
...
final performanceOptimizedMedicationsProvider =
...
final performanceOptimizedFilteredMedicationsProvider =
    Provider<AsyncValue<List<Medication>>>((ref) {
      final medicationsAsync = ref.watch(
        performanceOptimizedMedicationsProvider,
      );
...
List<Medication> _performEfficientFiltering(
  ...
  return medications.where((medication) {
    ...
    if (searchTerm.isNotEmpty) {
      ...
      final genericMatch =
          medication.genericName?.toLowerCase().contains(searchLower) ?? false;
...
final medicationPerformanceProvider =
    StateNotifierProvider<
      MedicationPerformanceNotifier,
      MedicationPerformanceState
    >((ref) {
...
class MedicationPerformanceNotifier
    extends StateNotifier<MedicationPerformanceState> {
  MedicationPerformanceNotifier()
    : super(MedicationPerformanceState(lastUpdate: DateTime.now()));
  ...
  void recordOperation(String operation, Duration duration) {
    ...
    _logger.debug(
      'Recorded operation: $operation took ${duration.inMilliseconds}ms',
    );
  }
  ...
  Map<String, double> getAverageOperationTimes() {
    ...
    for (final operation in state.operationTimes.keys) {
      final totalTime = state.operationTimes[operation]!.inMilliseconds
          .toDouble();
...
final performanceMedicationSearchTermProvider = StateProvider<String>(
  (ref) => '',
);
final performanceMedicationFormFilterProvider = StateProvider<MedicationForm?>(
  (ref) => null,
);
final performanceMedicationActiveFilterProvider = StateProvider<bool?>(
  (ref) => null,
);
