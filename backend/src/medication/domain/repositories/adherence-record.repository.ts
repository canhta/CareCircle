...
export abstract class AdherenceRecordRepository {
  ...
  abstract findTakenDoses(
    userId: string,
    startDate?: Date,
    endDate?: Date,
  ): Promise<AdherenceRecord[]>;
  abstract findMissedDoses(
    userId: string,
    startDate?: Date,
    endDate?: Date,
  ): Promise<AdherenceRecord[]>;
  abstract findSkippedDoses(
    userId: string,
    startDate?: Date,
    endDate?: Date,
  ): Promise<AdherenceRecord[]>;
  ...
  // Time-based queries
  abstract findDosesForDate(
    userId: string,
    date: Date,
  ): Promise<AdherenceRecord[]>;
  abstract findUpcomingDoses(
    userId: string,
    withinHours: number,
  ): Promise<AdherenceRecord[]>;
  ...
  // Streak calculations
  abstract getCurrentAdherenceStreak(
    userId: string,
    medicationId?: string,
  ): Promise<number>;
  abstract getLongestAdherenceStreak(
    userId: string,
    medicationId?: string,
  ): Promise<number>;
  abstract getStreakHistory(
    userId: string,
    medicationId?: string,
  ): Promise<
    Array<{
  ...
  // Pattern analysis
  abstract findAdherencePatterns(userId: string): Promise<
    Array<{
      dayOfWeek: number;
      timeOfDay: number;
      adherenceRate: number;
  ...
  >;
  abstract findMissedDosePatterns(userId: string): Promise<
    Array<{
      medicationId: string;
  ...
  abstract getMedicationAdherenceRanking(userId: string): Promise<
    Array<{
      medicationId: string;
      adherenceRate: number;
      totalDoses: number;
  ...
  abstract findRecentlyTaken(
    userId: string,
    hours: number,
    limit?: number,
  ): Promise<AdherenceRecord[]>;
  abstract findRecentlyMissed(
    userId: string,
    days: number,
    limit?: number,
  ): Promise<AdherenceRecord[]>;
  ...
  abstract findRecordsNeedingReminders(
    userId: string,
  ): Promise<AdherenceRecord[]>;
  ...
  abstract getDailyAdherenceCount(
    userId: string,
    date: Date,
  ): Promise<{
    scheduled: number;
