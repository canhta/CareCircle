...
export class CareGroupService {
  ...
  async createCareGroup(
    ...
    const careGroupData = CareGroupEntity.create({
      ...
      inviteCode: createDto.inviteCode,
      inviteExpiration: createDto.inviteExpiration,
    });
    ...
    const careGroup = await this.careGroupRepository.create(
      new CareGroupEntity(
        ...
        careGroupData.inviteCode,
        careGroupData.inviteExpiration,
        new Date(),
        new Date(),
      ),
    );
  ...
  async deleteCareGroup(groupId: string, userId: string): Promise<void> {
    ...
    if (!careGroup.isCreatedBy(userId)) {
      throw new ForbiddenException(
        'Only the creator can delete the care group',
      );
    }
  ...
  async updateMember(
    ...
    // Transform boolean flags to permissions array
    const permissions: string[] = [];
    if (updates.canInviteMembers) permissions.push('INVITE_MEMBERS');
    if (updates.canManageTasks) permissions.push('MANAGE_TASKS');
    if (updates.canViewHealthData) permissions.push('VIEW_HEALTH_DATA');

    const entityUpdates: Partial<CareGroupMemberEntity> = {
      role: updates.role,
      customTitle: updates.customTitle,
      permissions: permissions.length > 0 ? permissions : undefined,
    };

    return this.memberRepository.update(memberId, entityUpdates);
  }
